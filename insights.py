...
Salva data/insights.json (mais recente) e data/insights/YYYY-MM-DD.json (histórico 30 dias)
"""
...
HISTORY_DIR       = Path('data/insights')
...
def analyze_post(post):
    ...
    caption = post.get('caption', '')  # caption completa
...
IMPORTANTE: Analise a legenda COMPLETA acima. Não considere a legenda como cortada ou incompleta.
...
{{
  ...
  "legenda": {{"nota": "<bom|medio|ruim>", "analise": "<análise em 1-2 frases baseada na legenda COMPLETA>", "sugestao": "<sugestão prática>"}},
...
def generate_summary(posts_analyzed):
    ...
    # Resumo por patrocinador
    sponsor_stats = {}
    for p in posts_analyzed:
        for tag in (p.get('tags') or []):
            if tag not in sponsor_stats:
                sponsor_stats[tag] = {'posts':0,'views':0,'likes':0,'comments':0}
            sponsor_stats[tag]['posts']    += 1
            sponsor_stats[tag]['views']    += p.get('views',0)
            sponsor_stats[tag]['likes']    += p.get('likes',0)
            sponsor_stats[tag]['comments'] += p.get('comments',0)
...
Melhor post: {best.get('caption','')[:200]}
Alertas: {len(alerts)}
...
{{
  ...
  "insight_principal": "<o insight mais importante do dia em 2 frases>"
    ...
    try:
        ...
        summary['sponsor_stats']        = sponsor_stats
        return summary
    except Exception as e:
...
def cleanup_old_history():
    """Remove arquivos de insights com mais de 30 dias"""
    ...
    for f in HISTORY_DIR.glob('*.json'):
        try:
            ...
            if date < cutoff:
                f.unlink()
                print(f"  Removido histórico antigo: {f.name}")
        except ValueError:
            pass
...
def main():
    ...
    if not posts:
        ...
        for i, post in enumerate(posts):
            print(f"  Analisando {i+1}/{len(posts)}: {post.get('caption','')[:60]}...")
    ...
    if posts:
        s = insights['summary']
        print(f"Score médio: {s.get('score_medio')} | Posts: {s.get('total_posts')} | Alertas: {s.get('total_alertas')}")
